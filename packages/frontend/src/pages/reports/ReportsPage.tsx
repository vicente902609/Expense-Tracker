...
export const ReportsPage = ({ categoryPalette }: ReportsPageProps) => {
      ...
      <PageHeaderRow>
        <Box sx={{ minWidth: 0, flex: { sm: "1 1 0%" } }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
            See spending by month and by category. Use a preset or choose any date range that fits how you review your
            money.
          </Typography>
        ...
        <ChartCard>
          ...
          <Box sx={{ p: 2 }}>
                  ...
                  return (
                    <Stack key={entry.key} sx={{ flex: 1, minWidth: 0, alignItems: "center" }} spacing={1}>
                      <Tooltip title={`${entry.label}: ${formatCurrency(entry.total)}`} arrow>
                        <ChartBar $isActive={index === series.length - 1} $heightPx={barHeight} />
                      </Tooltip>
            ...
            <Typography variant="body2" sx={{ mt: 1.5, textAlign: "right", color: "text.secondary" }}>
              Total {formatCurrency(periodTotal)} · Avg {formatCurrency(averagePerBucket)} per month
            </Typography>
          </Box>
        </ChartCard>
        ...
        <ChartCard>
          ...
          <Stack spacing={1.5} sx={{ p: 2 }}>
            ...
            {categoryRows.map((row) => {
              ...
              const color = getCategoryColor(row.categoryId, categoryPalette);
              ...
              return (
                <Stack key={row.categoryId} direction="row" spacing={1.5} alignItems="center">
                  ...
                  <CategoryBarTrack>
                    <CategoryBarFill $percent={percent} $color={color} />
                  </CategoryBarTrack>
                  <Typography sx={{ minWidth: 72, textAlign: "right", fontWeight: 700, fontSize: 14, flexShrink: 0 }}>
