...
export const initialSchemaMigration: Migration = {
  ...
  description: "Create core collections and indexes for auth, expenses, goals, budgets, and migration tracking",
  up: async (database) => {
    ...
    await database.createCollection("budgetPlans").catch(() => undefined);
    ...
    await Promise.all([
      ...
      database.collection("budgetPlans").createIndex({ userId: 1 }, { unique: true, name: "budget_user_unique" }),
